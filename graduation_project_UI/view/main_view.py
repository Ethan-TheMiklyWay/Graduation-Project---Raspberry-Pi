...
class MainView(Frame):
    ...
    def help(self):
        ...
        middle = Canvas(self.middle_area, width=1000, height=300, bg="#FFFFFF", highlightthickness=0)
        middle.grid(row=0)
        text = '本软件为本科毕业设计软件\n请在控制命令选项卡中点击自己需要的功能，或者输入控制台命令\n'
        middle.create_text(370, 70, text=text, fill="#111111", font="Times 18")

        text = '作者：张靖祥，中国农业大学，计算机172班\n邮箱：1967527237@qq.com\n'
        middle.create_text(180, 130, text=text, fill="#111111", font="Times 12")
    ...
    def bottom_cmd(self):
        ...
        cmd_text = Text(bottom, bd="1", font="Times 12", height=12, width=124, bg="Black", fg="White",
    ...
    def middle_label_init(self):
        middle = Canvas(self, width=1000, height=300, bg="#FFFFFF", highlightthickness=0)
    ...
    def middle_button(self):
        self.middle_area.delete("all")
        middle_temp = Canvas(self.middle_area, width=1000, height=300, bg="#FFFFFF", highlightthickness=0)
        ...
        link_btn = Button(middle, bd=0, bg="#007DFA", height=2, width=20,
                          text="连接树莓派", anchor='center', font="20", fg="White",
                          activebackground="#007DFA", activeforeground="White",
                          command=lambda x=0: self.cmd_line_input("link -start"))
        link_btn.bind("<Enter>", lambda event: link_btn.configure(bg="#228EFB"))
        link_btn.bind("<Leave>", lambda event: link_btn.configure(bg="#007DFA"))
        link_btn.grid(row=0, column=2)

        cmd_text = Label(middle, text="  ", width=5, height=2, justify='left',
                         font="Times 1", fg='White', bg='White', padx=20, pady=10)
        cmd_text.grid(row=1, column=0, columnspan=3)

        disconnected_btn = Button(middle, bd=0, bg="#007DFA", height=2, width=20,
                          text="断开连接", anchor='center', font="20", fg="White",
                          activebackground="#007DFA", activeforeground="White",
                          command=lambda x=0: self.cmd_line_input("link -close"))
        disconnected_btn.bind("<Enter>", lambda event: disconnected_btn.configure(bg="#228EFB"))
        disconnected_btn.bind("<Leave>", lambda event: disconnected_btn.configure(bg="#007DFA"))
        disconnected_btn.grid(row=2, column=0)

        state_btn = Button(middle, bd=0, bg="#007DFA", height=2, width=20,
                                  text="连接状态", anchor='center', font="20", fg="White",
                                  activebackground="#007DFA", activeforeground="White",
                                  command=lambda x=0: self.cmd_line_input("link -state"))
        state_btn.bind("<Enter>", lambda event: state_btn.configure(bg="#228EFB"))
        state_btn.bind("<Leave>", lambda event: state_btn.configure(bg="#007DFA"))
        state_btn.grid(row=2, column=1)

        get_btn = Button(middle, bd=0, bg="#007DFA", height=2, width=20,
                                  text="获取数据", anchor='center', font="20", fg="White",
                                  activebackground="#007DFA", activeforeground="White",
                                  command=lambda x=0: self.cmd_line_input("get -all"))
        get_btn.bind("<Enter>", lambda event: get_btn.configure(bg="#228EFB"))
        get_btn.bind("<Leave>", lambda event: get_btn.configure(bg="#007DFA"))
        get_btn.grid(row=2, column=2)
    ...
    def show_form(self, title, head, form):
        self.form_data["表格"] = [title, head, form]
        self.label_select["表格"] = None
        self.select_area_repaint()
        self.label_select_func["表格"] = self.show_form_paint
    ...
    def show_form_paint(self):
        ...
        middle = Canvas(self.middle_area, width=1000, height=300, bg="#FFFFFF", highlightthickness=0)
        middle.grid(row=0)
        title, head, form = self.form_data["表格"]

        head_text = Label(middle, text=str(title), width=5, height=2, justify='left',
                          font="Times 16", fg='#111111', bg='White', padx=20, pady=10)
    ...
    def close_form(self):
        self.form_data.pop("表格")
        self.label_select.pop("表格")
        self.label_select_func.pop("表格")
        self.select_area_repaint()
        self.middle_button()
        self.v.set(0)
        pass
