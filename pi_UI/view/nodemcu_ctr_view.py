...
class NodemcuCtrMainView(Frame):
    ...
    def save(self):
        ...
        status = self.node_status_value.get()
        if status == "打开":
            status = 1
        else:
            status = 0
    ...
    def refresh(self):
        ...
        for line in self.info:
            if line["name"] == name:
                ...
                if status == 1:
                    self.node_status_value.set("打开")
                else:
                    self.node_status_value.set("关闭")
...
def start_nodemcu_ctr_main_view(line, main_frame):
    nodemcu_root_tk = Tk()
    ...
    NodemcuCtrMainView(master=nodemcu_root_tk, info=line, main_frame=main_frame).pack()
